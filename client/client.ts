 ...
 */
import { ShodanHTTPClient } from "../utilities/http_requests.ts";
...
export class ShodanClient {
    ...
    public async searchHostWithIP<T = unknown>(ip: string, options?: IpSearchOptions): Promise<T> {
    ...
    public async countHostsWithQuery<T = unknown>(query: string, options?: CountSearchOptions): Promise<T> {
                
        return await this.ShodanHTTPClientObject.get<T>(`shodan/host/count?${this.buildQueryParams({ query, ...options })}`);

    }

    public async searchHostsWithQuery<T = unknown>(query: string, options?: SearchOptions): Promise<T> {
    ...
    public async submitScanRequest<T = unknown>(body: PostShodanRequestBody): Promise<T> {
    ...
    public async getSavedSearchQueries<T = unknown>(options?: SavedSearchQueriesOptions): Promise<T> {

        return await this.ShodanHTTPClientObject.get<T>(`shodan/query?${this.buildQueryParams(options)}`)

    }

    public async getDirectoryWithSavedSearchQueries<T = unknown>(query: string, options?: DirectorySearchOptions): Promise<T> {

        return await this.ShodanHTTPClientObject.get<T>(`shodan/query/search?${this.buildQueryParams({ query, ...options })}`);

    }

    public async getSavedSearchQueryTags<T = unknown>(options?: SavedQueryTagsOptions): Promise<T> {
    ...
    public async getDNSRecords<T = unknown>(domain: string, options?: DomainDNSOptions): Promise<T> {
